...
func _process(delta):
	...
	if parent.climbing == true:
		if climbing_start == true:
			left_hand_target_position = Vector2(parent.get_global_position().x, parent.get_global_position().y - 40)
			...
			if $"IK Targets/Right_Hand_target".position.y < -50:
				right_hand_target_position.y += 5
			if $"IK Targets/Right_Hand_target".position.x > 40:
				right_hand_target_position.x -= 1
			elif $"IK Targets/Right_Hand_target".position.x < 10:
				right_hand_target_position.x += 1
			#Keeps left hands within bounds
			if $"IK Targets/Left_hand_target".position.y < -50:
				left_hand_target_position.y += 5
			if $"IK Targets/Left_hand_target".position.x < -40:
...
func set_climbing_left_target_point():
	if $"IK Targets/Left_hand_target".position.y > -150:
		...
		left_global_posit.x += parent.velocity.x/4
